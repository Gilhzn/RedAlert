...
## שילוב קובצי BlendSwap (.blend)

הסביבה של קלוד לא יכולה להוריד מ־BlendSwap (נדרש חשבון + האתר חסום), אבל יש
מסלול פשוט: מורידים אצלכם ומעבירים דרך הריפו — קלוד פותח ‎.blend ישירות
ב־Blender ומשלב במפה (רינדור Cycles לדמו + GLB ל־Unity).

1. הורידו את הקובץ מ־BlendSwap (התחברו לחשבון). **רשמו את הרישיון שמופיע
   בעמוד** (CC-0 / CC-BY / CC-BY-SA / GPL — שני האחרונים בעייתיים למשחק סגור).
2. שמרו את ה־.blend בתיקייה `assets_src/blendswap/` בריפו.
3. ‎`git add . && git commit -m "add blendswap asset" && git push`
4. כתבו לקלוד מה הקובץ ומה תפקידו (עיטור מפה / מבנה / רכב) — והוא ישלב.
